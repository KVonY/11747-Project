...
def cal_acc(cand_probs, answer, batch_size):
    ...
    for acc_i in range(batch_size):
        acc_ans_list.append(cand_a[acc_i].item())

        if cand_a[acc_i] == answer[acc_i]: 
            acc_cnt += 1
            acc_correct_list.append(1)
        else:
            acc_correct_list.append(0)

    acc_prob = acc_cnt / batch_size

    return acc_prob, acc_ans_list, acc_correct_list
...
def evaluate_result(config, dev_data, coref_model, sen_cut_dev):
    ...
    for batch_i in range(n_batch_data):
        ...
        acc_prob_i, acc_ans_list_i, acc_correct_list_i = cal_acc(cand_probs_dev, answer_dev, config['batch_size'])

        acc_ans_list += acc_ans_list_i
        acc_correct_list += acc_correct_list_i

        acc_dev_list.append(acc_prob_i)
    
    acc_dev_whole = sum(acc_dev_list) / n_batch_data
    print("---- dev acc whole: " + str(round(acc_dev_whole, 4)))

    print(sum(acc_correct_list))
    print(len(acc_ans_list))
    # print(acc_ans_list)

    with open("dev_whole_pred.txt", 'w') as outfile:  # index, correct? (0 or 1), pred_ans_index
        for i in range(len(acc_ans_list)):
            out_str = str(i) + "\t" + str(acc_correct_list[i]) + "\t" + str(acc_ans_list[i])
...
def main():
    ...
    evaluate_result(config, dev_data, coref_model, sen_cut_dev)
